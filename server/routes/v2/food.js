...
const saveStall = async (req, res, create) => {
  ...
  if (!validId(id) || !validId(slug) || name.length < 2 || name.length > 255 || !['food', 'parlour'].includes(type) || !['draft', 'active', 'inactive'].includes(status)) return sendError(res, 400, 'Provide a valid id, slug, name, stall type, and status.');
  if (![req.body.shortDescription, req.body.description, req.body.logoImageUrl, req.body.coverImageUrl].every(boundedText) || !jsonArray(req.body.operatingHours) || !jsonObject(req.body.contact) || !jsonObject(req.body.metadata)) return sendError(res, 400, 'Stall content is invalid.');
