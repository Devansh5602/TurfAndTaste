...
- Booking now requests a fresh server quote after facility/date/slot changes, uses it for the visible total/deposit, and blocks payment/submission until it is current. Quote context accompanies the legacy submission payload, but the backend has not yet enforced it; next add a server-side quote reference and amount enforcement while preserving authenticated walk-ins.
